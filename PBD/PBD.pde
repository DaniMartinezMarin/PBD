...
void setup(){
  ...
  suelo = new Suelo(new PVector(0,0,0), scale_px);
...
void draw(){
  ...
  system.apply_gravity(new PVector(0.0,-0.11,0.0));
  aplica_viento();
  system.run(dt);  
  display();
  stats();
...
void display(){
  ...
  //system.objeto.display(scale_px);
  suelo.display(scale_px);
